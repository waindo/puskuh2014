	   ...
	   var maxValue,minValue;
		...
		//	alert(data[i].$startX);
			...
			if(value<0||(this._settings.yAxis&&value===0)){
				//alert('test');
				this.renderTextAt("middle", true, x0+10,y0+Math.floor(barWidth/2),this._settings.label(data[i]));
				continue;
			}
			...
			/*sets a bar label*/
			//alert(Math.floor(data[i].$startY));
			this.renderTextAt("middle",true,x0+(points[0]-x0)/2-1, y0+(points[1]-y0)/2, this._settings.label(data[i]));
	...
	*/
	pvt_render_stackedBar:function(ctx, data, point0, point1, sIndex, map){
		...
		for(var i=0; i < data.length;i ++){
			...
			/*sets a bar label*/
			//alert((points[1]+(y0-points[1])/2)-7);
			this.renderTextAt(false, true, x0+Math.floor(barWidth/2),(points[1]+(y0-points[1])/2)-7,this._settings.label(data[i]));
			/*defines a map area for a bar*/
