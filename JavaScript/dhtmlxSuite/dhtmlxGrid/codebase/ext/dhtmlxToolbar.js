...
document.write( "<link href=\"themes/" + DHTML_SUITE_THEME + 
 							"/css/dhtmlxtoolbar.css\" rel=stylesheet type=\"text/css\">" );
...
dhtmlXToolbarObject.prototype.addItem = function( item, pos )
{
	...
	if ( this.extraMode )
	{
		...
		this.firstCell.parentNode.parentNode.insertBefore( tr, ( pos ==
	this.itemsCount ) ? this.firstCell.parentNode : this.items[pos].getTopNode()
	.parentNode );
		item.getTopNode().style.width = '100%';
	}
	else
	{
		this.firstCell.parentNode.insertBefore( item.getTopNode(), ( pos ==
	this.itemsCount ) ? this.firstCell : this.items[pos].getTopNode());
...
dhtmlXToolbarObject.prototype._parseXMLTree = function( that, node )
{
	...
	for ( var i = 0; i < node.childNodes.length; i++ )
	{
		...
		if ( localItem.nodeType == 1 )
		{
			...
			if (( lAtr.disableImage ) && ( that.sysGfxPath ))
				lAtr.disableImage = that.sysGfxPath + lAtr.disableImage;
 ...
 *     @topic: 0
 */
dhtmlXToolbarObject.prototype.setToolbarCSS = function( table, title, button,
	text )
{
	this.tableCSS = table;
	this.titleCSS = title;
 																...
 																className, disableImage )
{
	...
	if ( typeof( src ) != "object" )
		src = this._arg2obj( arguments, ["src", "width", "height", "action", "id", 
											"tooltip", "className", "disableImage"] );
	...
	this.src= DHTMLSuite.configObj.imagePath + 'toolbar-icons/' + src.src;   
	this.disableImage= DHTMLSuite.configObj.imagePath + 'toolbar-icons/' + src.disableImage;
	/*
	this.src = src.src;
	...
	*/
	...
	td.align = "center";
	td.vAlign = "middle";
	td.innerHTML = "<img src='" + this.src + "' border='0' title='" + this.tooltip
 ...
 *     @topic: 0
 */
function dhtmlXImageTextButtonObject( src, content, width, height, action, id,
 																		tooltip, className, textClassName,
 																		disableImage )
{
	if ( typeof( src ) != "object" )
		src = this._arg2obj( arguments, ["src", "content", "width", "height", 
											"action", "id", "tooltip", "className", "textClassName", 
											"disableImage"] );
	...
	if ( !src.textmode )
	{
		...
		td.innerHTML = "<table title='" + this.tooltip + 
	"' width='100%' height='100%' cellpadding='0' cellspacing='0'><tr><td valign='center' align='center' valign='middle'  style='padding-left:4px; padding-right:4px;'><img src='" + this.src + "' border='0'></td><td width='100%' style='padding-left:5px' align='left' valign='center'  class='" + this.textClassName + "'>" + src.content + "</td></tr></table>";
	...
	else
	{
		...
		td.innerHTML = "<table title='" + this.tooltip + 
	"' width='100%' height='100%' cellpadding='0' cellspacing='0'><tr><td align='center' valign='middle'><img src='" + this.src + "' border='0'  style='padding-left:2px; padding-right:2px;'></td></tr><tr><td width='100%' align='center' class='" + this.textClassName + "'>" + src.content + "</td></tr></table>";
		this.textTag = td.childNodes[0].rows[1].cells[0];
	}
	td.className = this.className;
	td.objectNode = this;
	this.imageTag =
	td.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0];
	this.enable();
	if ( src.mouseover )
	{
		this._mvImage = src.mouseover;
 ...
 *     @topic: 0
 */
function dhtmlXImageTextButtonXPObject( src, content, width, height, action, id,
 																			tooltip, className, textClassName,
 																			disableImage )
{
	if ( typeof( src ) != "object" )
...
{
	if ( typeof( id ) != "object" )
		id = this._arg2obj( arguments, ["id", "valueList", "displayList", "action", 
	...
	{
		sel.options[sel.options.length] = new Option( id.option[i].content,
	id.option[i].value );
	}
...
{
	if (( !this.objectNode.persAction ) || ( this.objectNode.persAction(
...
{
	this.selElement.options[this.selElement.options.length] = new Option( display,
	value );
};
/**  
 ...
 *     @topic: 6
 */
dhtmlXSelectButtonObject.prototype.setOptionValue = function( oldValue,
	newValue )
{
 ...
 *     @topic: 0  
 */
function dhtmlXTwoStateButtonObject( id, src, content, width, height, action,
																		tooltip, className, textClassName,
																		disableImage, pressedState )
{
	if ( typeof( id ) != "object" )
		id = this._arg2obj( arguments, ["id", "src", "content", "width", "height", 
 										"action", "tooltip", "className", "textClassName", 
 										"disableImage", "pressedState"] );
	...
	this.disableImage= DHTMLSuite.configObj.imagePath + 'toolbar-icons/' + id.disableImage;
	...
	if ( id.src )
	{
		this.srcA= (DHTMLSuite.configObj.imagePath + 'toolbar-icons/' + id.src).split(','); 
	...
	td.innerHTML = "<table title='" + this.tooltip + 
	"'  width='100%' height='100%' cellpadding='0' cellspacing='0'><tr><td align='center' valign='middle'><img src='" + this.srcA[0] + "' border='0'  style='padding-left:2px;padding-right:2px;'></td><td width='100%' style='padding-left:5px' align='left' class='" + this.textClassName + "'>" + this.textP[0] + "</td></tr></table>";
	td.className = this.className;
	td.objectNode = this;
	this.imageTag =
	td.childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0];
 ...
 *     @topic: 0
 */
function dhtmlXSliderButtonObject( id, size, skin, vertical, step, limit,
																	init_pos, shift, className )
{
	if ( typeof( id ) != "object" )
		var conf = this._arg2obj( arguments, ["id", "size", "skin", "vertical", 
 													"limit", "init_pos", "shift", "className"] );
	else
		var conf = id;
