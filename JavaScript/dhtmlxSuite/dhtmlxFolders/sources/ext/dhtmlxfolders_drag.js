...
*/
	...
	*/
	dhtmlxFolders.prototype._createDragNode=function(htmlObject,e){
		...
		this._idpullCol[id].parentObject={id:id, ex:this, treeNod:{_nonTrivialNode:this._dragInTree,_nonTrivialRow:this._dragInGrid}};
	...
	*/
	dhtmlxFolders.prototype._drag=function(source,from,target,lastLanding){
		...
		for (var j=0; j<from._dragged.length; j++){
			...
			if (from.mytype=="tree" ){
				/*FIXME*/
				this.addItem(sid,from.getItemText(sid),{id:target._exp_id,mode:this._dndPos});
				from.deleteItem(sid);
			} else if (from.rowsBuffer){
				/*FIXME*/
				this.addItem(sid,from.cells(sid,0).getValue(),{id:target._exp_id,mode:this._dndPos});
				from.deleteRow(sid);
			} else {
	...
	*/
	dhtmlxFolders.prototype._dragIn=function(htmlObject,shtmlObject,x,y){
		...
		if (!htmlObject.itemObj) return htmlObject; //unknown object draged in - confirm
	...
	*/
	dhtmlxFolders.prototype._dragOut=function(htmlObject){    
		this.hideDnDSelection(true);
		if (!htmlObject.itemObj) return; //drag over body
